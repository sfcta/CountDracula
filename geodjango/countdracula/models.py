...
class TurnCount(models.Model):
    ...
    from_street         = models.ManyToManyField(StreetName, help_text="Street from which turn originates", related_name="tc_from_street+")
    from_dir            = models.CharField(max_length=5, choices=Directions, help_text="Direction going into turn")
    to_street           = models.ManyToManyField(StreetName, help_text="Street to which the turn is destined", related_name="tc_to_street+")
    to_dir              = models.CharField(max_length=5, choices=Directions, help_text="Direction coming out of turn")
    intersection_street = models.ManyToManyField(StreetName, help_text="Cross street to identify the intersection", related_name="tc_int_street+")
    int_id              = models.ManyToManyField(Node, help_text="Intersection id", related_name="tc_int_id+")
...
class MainlineCount(models.Model):
    ...
    period_minutes      = models.IntegerField("Period the count was taken (minutes)")
    vehicle_type        = models.IntegerField(choices=VehicleTypes)
    on_street           = models.ManyToManyField(StreetName, help_text="The street with the count", related_name="mc_on_street+")
    on_dir              = models.CharField(max_length=5, choices=Directions, help_text="Direction of count")
    from_street         = models.ManyToManyField(StreetName, help_text="Cross street before count", related_name="mc_from_street+")
    to_street           = models.ManyToManyField(StreetName, help_text="Cross street after count", related_name="mc_to_street+")
